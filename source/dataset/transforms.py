...
class AugmentationPipeline:
    ...
    ) -> tuple[NDArray | torch.Tensor, NDArray | torch.Tensor, NDArray | torch.Tensor, NDArray | torch.Tensor, NDArray | torch.Tensor]:
    ...
    def random_square_crop(
        ...
        # Apply crop to image
        cropped_image = self._crop_image_with_padding(image, patch, (ch, cw), fill_value)
        ...
        return cropped_image, cropped_boxes, cropped_keypoints, cropped_labels, cropped_weights
    ...
    def letter_box(
        ...
        image = cv2.copyMakeBorder(
            image, 
            pad_h, pad_h_bottom,  # top, bottom
            pad_w, pad_w_right,   # left, right
            cv2.BORDER_CONSTANT, 
            value=0
        )
    ...
    def to_float32_tensor(
        ...
        image = torch.from_numpy(np.ascontiguousarray(image)).permute(2, 0, 1).to(torch.float32)
